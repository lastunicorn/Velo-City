...
internal class MergeAnalysis
{
    ...
    {
        if (PreviousVacation is VacationOnce or VacationDaily && PreviousVacation.HourCount == Hours)
            CanMergeLeft = true;

        if (NextVacation is VacationOnce or VacationDaily && NextVacation.HourCount == Hours)
            CanMergeRight = true;
    }
}